...
class receiver:   #Receiver Class Described
    ...
    def generate_prime_pair(self,b=4):
...
def additive_sharing(p,m,g):
    ...
    for i in range(p-1):
        additive_shares_new.append(random.randrange(0,m+1))     #FIX to pick values 0 to m and roll around with modulo - DONE
...
def proac_refresh():
    ...
    n_old = len(old_shares)
    new_shares = [0 for _ in range(n_old)]
    for i in old_shares:
        share_div = additive_sharing(n_old,i,0)
        new_shares = [(a+b)%n for a,b in zip(new_shares,share_div)]     
...
def signature_verify(sign):
    ...
    global partial_signatures
    global n
    print("PRODUCT:::::::::",functools.reduce(mul, partial_signatures, 1)%n)
...
if __name__ == "__main__":
    ...
    proactivizer.proactive_timer(additive_shares,n)
